if (document.querySelector(".comment-barrage")) {
  var commentBarrageConfig = {
      ...
      avatarD: GLOBAL_CONFIG.commentBarrageConfig.avatarD,
    },
  ...
  function popCommentBarrage(e) {
    ...
    (a.className = "comment-barrage-item"),
      ...
      a.querySelectorAll("anzhiyu pre").forEach((e) => {
        let a = document.createElement("span");
        (a.innerText = "【代码】"), e.parentNode.replaceChild(a, e);
      }),
      a.querySelectorAll("anzhiyu img").forEach((e) => {
        if (!e.classList.contains("tk-owo-emotion")) {
          e.style.display = "none";
          let a = document.createElement("span");
          (a.innerText = "【图片】"), e.parentNode.replaceChild(a, e);
        }
      }),
    ...
    (e.className = "comment-barrage-item out"),
      setTimeout(() => {
        commentBarrageConfig.dom &&
          commentBarrageConfig.dom.contains(e) &&
  ...
  const e = new IntersectionObserver(
      (e) => {
        const a = document.querySelector(".comment-barrage"),
          r = document.getElementById("post-comment");
        e.forEach((e) => {
          r &&
            a &&
            document.body.clientWidth > 768 &&
            (a.style.bottom = e.isIntersecting
              ? `-${200 * commentBarrageConfig.maxBarrage}px`
              : "0");
        });
      },
      { root: null, rootMargin: "0px", threshold: 0 }
    ),
    a = document.getElementById("post-comment");
  a && e.observe(a),
    initCommentBarrage(),
    "false" !== localStorage.getItem("commentBarrageSwitch")
