      ...
      caches.match('https://id.v3/').then(function(response) {
        if (response) {
          ...
          response.json().then(function(data) {
            anzhiyuPopupManager && anzhiyuPopupManager.enqueuePopup('通知📢', `已刷新缓存，更新为${data.global + "." + data.local}版本最新内容。最好手动清理本地缓存保持同步！`, null, 5000);
          });
        } else {
...
/** 跳过处理番剧封面 和 meting-api,此类api不适用于service worker */
...
/**meting-api
 * 
 * 默认 https://api.i-meto.com/meting/api
...
module.exports.cacheRules = {
  simple: {
    ...
    match: (url, $eject) => {
      ...
      const allowedPaths = ["/404.html", "/css/index.css","/index.html"];
      return url.host === allowedHost && (allowedPaths.includes(url.pathname) || url.pathname.startsWith("/"));
    },
  },
  cdn: {
    clean: true,
